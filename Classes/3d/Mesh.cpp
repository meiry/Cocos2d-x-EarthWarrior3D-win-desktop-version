...
bool ObjMeshParser::parseVertex(std::istream& lineStream, ObjMeshData& meshData)
{
    ...
    //check
    if(lineStream.bad() || !lineStream.eof() || !std::isspace(whiteSpacex_y,loc) || !std::isspace(whiteSpacey_z,loc))
    {
        return false;
    }
...
bool ObjMeshParser::parseTextureVertex(std::istream& lineStream, ObjMeshData& meshData)
{
    ...
    //check
    if(lineStream.bad() || !lineStream.eof() || !std::isspace(whiteSpaceu_v,loc))
    {
        return false;
    }
    v = 1 - v;
...
bool ObjMeshParser::parseFace(std::istream& lineStream, ObjMeshData& meshData)
{
    ...
    while (!lineStream.bad() && !lineStream.eof())
    {
        ...
        if(!lineStream.eof())
        {
            ...
            if(!std::isspace(whiteSpace,loc)) return false;
        }
        ...
        if(!stringstream.eof())
        {
            ...
            else
            {
                ...
                if(keyword == "v")
                {
                    ...
                    //check and parse
                    if(!std::isspace(whiteSpace,loc) || stringstream.bad() || !parseVertex(stringstream, meshData))
                    {
                        logParseError(line_number,line);
                    }
                }
                else if(keyword == "vt")
                {
                    stringstream>>whiteSpace>>std::ws;
                    //check
                    if(!std::isspace(whiteSpace,loc) || stringstream.bad() || !parseTextureVertex(stringstream, meshData))
                    {
                        logParseError(line_number,line);
                    }
                }
                else if(keyword == "vn")
                {
                    stringstream>>whiteSpace>>std::ws;
                    //check
                    if(!std::isspace(whiteSpace,loc) || stringstream.bad() || !parseNormal(stringstream, meshData))
                    {
                        logParseError(line_number,line);
                    }
                }
                else if((keyword == "f") || (keyword == "fo"))
                {
                    stringstream>>whiteSpace>>std::ws;
                    //check
                    if(!std::isspace(whiteSpace,loc) || stringstream.bad() || !parseFace(stringstream, meshData))
                    {
                        logParseError(line_number,line);
                    }
                }
